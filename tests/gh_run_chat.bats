...
setup() {
	...
	eval "$(
		...
		declare -f _parse_chat_args _parse_run_chat_args _extract_ai_arg _show_run_chat_help _gh_run_chat \
			_prepare_run_chat_context _prepare_run_context \
...
@test "_gh_run_chat: calls _cmd_chat with rendered prompt and session args" {
	...
	[[ "$output" == *"PROMPT:"* ]]
	[[ "$output" == *"Test Run"* ]]
	[[ "$output" == *"/.local/state/gh/ai/sessions/run-12345678"* ]]
}
...
@test "_gh_run_chat: forwards _GH_AI_ARGS to _cmd_chat via claude" {
	_setup_chat_mocks
