...
description: >
  ...
  then applies the edit after user confirmation. Trigger when the user says
  "edit PR #N to add a testing section", "update the PR body", "rewrite the
  description", "fix the PR title", "add a summary", "update the PR checklist",
...
2. Show the full updated PR framed with horizontal rules.
...
5. When the user confirms, save and apply:
   ...
   - Extract the title (first `# ...` line, without the `# ` prefix) → `${GH_CLAUDE_SESSION_DIR}/drafts/pr_title_draft.txt`
   - Write ONLY the body (everything after the title line) → `${GH_CLAUDE_SESSION_DIR}/drafts/pr_body_draft.md` — do NOT include the `# Title` line
   - `gh pr edit ${GH_PR_NUMBER} --title "$(cat ${GH_CLAUDE_SESSION_DIR}/drafts/pr_title_draft.txt | tr -d '\n')" --body-file ${GH_CLAUDE_SESSION_DIR}/drafts/pr_body_draft.md`
...
- If `gh pr edit` fails, show the full error and suggest `gh auth status`.
