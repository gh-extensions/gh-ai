...
5. When the user confirms, ensure the drafts directory exists (`mkdir -p $GH_AI_SESSION_DIR/drafts`),
   write the final comment to `$GH_AI_SESSION_DIR/drafts/pr_comment_draft.md`, and run:
   `gh pr comment $GH_AI_PR_NUMBER --body-file $GH_AI_SESSION_DIR/drafts/pr_comment_draft.md`
