...
6. When the user confirms, run `mkdir -p $GH_CLAUDE_SESSION_DIR/drafts`, write the plan body to
   ...
   `printf '\n<!-- gh-claude:issue-plan issue=%s -->' "$GH_CLAUDE_ISSUE_NUMBER" >> $GH_CLAUDE_SESSION_DIR/drafts/issue_plan_draft.md`
...
8. Check if a plan comment already exists on the issue (use `--paginate` to search all comments).
   ...
   `gh api repos/{owner}/{repo}/issues/$GH_CLAUDE_ISSUE_NUMBER/comments --paginate | jq -s --arg n "$GH_CLAUDE_ISSUE_NUMBER" '[.[][] | select(.body | contains("<!-- gh-claude:issue-plan issue=\($n) -->"))] | last | .id // empty'`
   ...
   - If no comment is found: create a new one:
     `gh issue comment $GH_CLAUDE_ISSUE_NUMBER --body-file $GH_CLAUDE_SESSION_DIR/drafts/issue_plan_draft.md`
